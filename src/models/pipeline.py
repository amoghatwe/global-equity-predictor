...
class ModelPipeline:
    ...
    def _train_market(
        self, market: str, cv: TimeSeriesCrossValidator
    ) -> Dict[str, Any]:
        """
        ...
        X = self.data.drop(
            columns=[col for col in self.data.columns if "_return_3y" in col]
        )
        ...
        if len(X) < cv.min_train_months + 12:
            self.logger.error(
                f"Insufficient data for {market}: {len(X)} samples (need at least {cv.min_train_months + 12})"
            )
            return {}
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X)):
            ...
            self.logger.info(
                f"  Train: {len(X_train)} samples ({X_train.index[0]} to {X_train.index[-1]})"
            )
            self.logger.info(
    ...
    def _create_models(self) -> Dict[str, Any]:
        ...
        if self.model_type == "ensemble" or self.model_type == "all":
            ...
            models["arma"] = create_model("arma")
            models["arima"] = create_model("arima")
        elif self.model_type in ["linear", "rf", "xgb", "arma", "arima"]:
        ...
        for model_name in model_names:
            ...
            for metric_name in metric_names:
                ...
                if np.all(np.isnan(values_array)):
                    aggregated[model_name][metric_name] = np.nan
                    aggregated[model_name][f"{metric_name}_std"] = np.nan
                else:
                    aggregated[model_name][metric_name] = np.nanmean(values_array)
                    aggregated[model_name][f"{metric_name}_std"] = np.nanstd(
                        values_array
                    )
    ...
    def predict(self) -> Dict[str, pd.DataFrame]:
        ...
        X_latest = X_latest.drop(
            columns=[col for col in X_latest.columns if "_return_3y" in col]
        )
        ...
        for market in self.markets:
            ...
            for model_name, model in self.models[market].items():
                try:
                    ...
                    self.logger.error(
                        f"Prediction failed for {market}/{model_name}: {str(e)}"
                    )
            ...
            for model_file in market_dir.glob("*.pkl"):
                ...
                try:
                    ...
                    if model_name == "xgb":
                        ...
                        from .implementations import ARMAReturnModel

                        model_instance = ARMAReturnModel()
                    elif model_name == "arima":
                        from .implementations import ARIMAReturnModel

                        model_instance = ARIMAReturnModel()
                    else:
                        self.logger.error(f"Unknown model name: {model_name}")
                        continue

                    if model_name in ("arma", "arima"):
    ...
    def evaluate(self, results: Dict[str, Any]):
        ...
        for market, result in results.items():
            ...
            for model_name, metrics in cv_metrics.items():
                ...
                self.logger.info(
                    f"  {model_name:15s}: RMSE={rmse:6.2f}, "
