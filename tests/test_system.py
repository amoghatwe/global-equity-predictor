...
class TestFeatureEngineering:
    ...
    def test_feature_creation_sample(self):
        ...
        df = pd.DataFrame(
            {"price": 100 + np.arange(24), "gdp": 2.0 + np.random.randn(24) * 0.5},
            index=dates,
        )
...
class TestModels:
    ...
    def test_arma_model_creation(self):
        """Test ARMA model can be created."""
        from src.models.implementations import ARMAReturnModel

        model = ARMAReturnModel()
        assert model is not None
        assert model.name == "ARMA"
        assert model.arma_order == (1, 1)

    def test_arima_model_creation(self):
        """Test ARIMA model can be created."""
        from src.models.implementations import ARIMAReturnModel

        model = ARIMAReturnModel()
        assert model is not None
        assert model.name == "ARIMA"
        assert model.arima_order == (1, 1, 1)

    def test_arma_train_predict(self):
        """Test ARMA model train and predict with synthetic data."""
        from src.models.implementations import ARMAReturnModel
        ...
        y = pd.Series(np.cumsum(np.random.randn(n)) * 0.5, index=dates, name="target")
        y.iloc[-36:] = np.nan
        ...
        model = ARMAReturnModel(order=(1, 1))
        ...
        pred = model.predict(X_clean.iloc[-1:])
        assert len(pred) == 1

    def test_arima_train_predict(self):
        """Test ARIMA model train and predict with synthetic data."""
        from src.models.implementations import ARIMAReturnModel
        ...
        model = ARIMAReturnModel(order=(1, 1, 1))
        metrics = model.train(X_clean, y_clean)
    ...
    def test_create_model_factory(self):
        """Test create_model factory supports all model types."""
        from src.models.implementations import create_model

        for model_type in ["linear", "rf", "xgb", "arma", "arima"]:
...
class TestTimeSeriesCV:
    ...
    def test_cv_split_sizes(self):
        ...
        cv = TimeSeriesCrossValidator(
            n_splits=5, min_train_years=10, max_train_years=None
        )
